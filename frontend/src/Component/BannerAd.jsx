...
const BannerAd = ({ slot = "horizontal", className = "", category }) => {
  ...
  useEffect(() => {
    if (ads && ads.length > 0) {
      // Prioritize an ad that matches the category exactly
      const categorySpecificAd = ads.find(a => a.category === category);
      // Fallback to the latest ad (which could be global)
      setCurrentAd(categorySpecificAd || ads[0]);
    } else {
      // Fallback to static config if no dynamic ads are available at all
