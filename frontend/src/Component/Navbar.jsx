...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    setCurrentDate(new Date().toLocaleDateString(i18n.language === 'bn' ? 'bn-BD' : 'en-US', options));
  ...
  return (
    <header className="w-full bg-white z-[70]">
        ...
        <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            <button 
              ...
              className="text-[10px] font-black uppercase tracking-widest text-gray-900 hover:text-orange-600 transition-colors border border-gray-200 px-2 py-1 rounded"
            >
              {t('languageToggle')}
            </button>
            ...
            {!user && (
              <Link
                to="/log-in"
                className="text-[10px] font-outfit font-black uppercase tracking-widest text-gray-900 hover:text-orange-600 transition-colors"
              >
                {t('signIn')}
              </Link>
            )}
      ...
      <div className="py-6 md:py-10 border-b border-gray-50">
        <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
          ...
          >
            ...
            />
            <span className="text-[10px] md:text-xs font-outfit font-black uppercase tracking-[0.4em] text-gray-400 mt-2 text-center">
          ...
          <div className="flex items-center gap-1 md:gap-3">
            <div
              className={`flex items-center transition-all duration-500 ease-out overflow-hidden ${isSearchOpen ? "w-[180px] sm:w-[300px] md:w-[400px] opacity-100 mr-2 md:mr-4" : "w-0 opacity-0"}`}
            >
              <form
              ...
              >
                ...
                <button
                  ...
                  className="absolute right-3 text-gray-300 hover:text-gray-900 transition-colors"
                >
            ...
            {!isSearchOpen && (
              <button
                ...
                className="p-3 hover:bg-gray-50 rounded-full transition-all text-gray-400 hover:text-gray-900 active:scale-90"
              >
            ...
            {user && (
              <div
                className={`relative group transition-all duration-300 ${isSearchOpen && window.innerWidth < 640 ? "hidden" : "block"}`}
              >
                <img
                  src={user?.profileImage || avatarImg}
                  alt="user"
                  className="w-10 h-10 md:w-11 md:h-11 rounded-full border-2 border-white ring-1 ring-gray-100 group-hover:ring-orange-600 transition-all cursor-pointer object-cover shadow-sm"
                />
                <div className="absolute hidden group-hover:block top-full right-0 pt-3 w-52 z-[100]">
                  <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
                    ...
                    <Link
                      to="/profile"
                      className="block px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors"
                    >
                      {t('journalistProfile')}
                    </Link>
                    {user.role === "admin" && (
                      <Link
                        to="/dashboard"
                        className="block px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-colors border-t border-gray-50"
                      >
                        {t('adminDashboard')}
                      </Link>
                    )}
      ...
      <nav
        className={`w-full z-[60] transition-all duration-300 border-b border-gray-100 ${scrolled ? "fixed top-0 bg-white/95 backdrop-blur-md shadow-lg py-2" : "relative bg-white py-0 md:py-4 hidden md:block"}`}
      >
        <div className="container mx-auto px-4 md:px-6 flex justify-between lg:justify-center items-center gap-6">
          {scrolled && (
            <Link
              to="/"
              className="flex-shrink-0 lg:absolute lg:left-6 transition-all animate-in fade-in slide-in-from-left-2"
              onClick={() => handleCategoryChange("")}
            >
          ...
          <ul className="hidden lg:flex items-center gap-10">
            ...
            {moreCategories.length > 0 && (
              <li className="relative group">
                <button
                  className={`text-[11px] font-outfit font-black uppercase tracking-[0.2em] py-2 flex items-center gap-1 transition-all ${moreCategories.some((c) => searchParams.get("category") === c.name) ? "text-orange-600" : "text-gray-900 hover:text-orange-600"}`}
                >
                  {t('more')}
                  <svg
                    className="w-3 h-3"
            ...
            <li>
              <NavLink
                to="/about-us"
                className={({ isActive }) =>
                  `text-[11px] font-outfit font-black uppercase tracking-[0.2em] transition-all duration-300 relative py-2 ${isActive ? "text-orange-600 after:w-full" : "text-gray-900 hover:text-orange-600 after:w-0"} after:content-[''] after:absolute after:bottom-0 after:left-0 after:h-[2px] after:bg-orange-600 after:transition-all hover:after:w-full`
                }
              >
                {t('about')}
              </NavLink>
          ...
          {scrolled && (
            <div className="flex items-center gap-2 md:gap-4 lg:absolute lg:right-6 animate-in fade-in slide-in-from-right-2">
              <button
                onClick={() => setIsSearchOpen(true)}
                className="p-2 md:p-2.5 text-gray-400 hover:text-orange-600 transition-colors"
              >
              ...
              {user ? (
                ...
                <Link
                  to="/log-in"
                  className="text-[9px] font-black uppercase tracking-widest text-gray-900 hover:text-orange-600 transition-colors"
                >
                  {t('signIn')}
                </Link>
              )}
              <button 
                onClick={() => i18n.changeLanguage(i18n.language === 'en' ? 'bn' : 'en')}
                className="text-[9px] font-black uppercase tracking-widest text-gray-900 hover:text-orange-600 transition-colors border border-gray-200 px-1.5 py-0.5 rounded"
              >
      ...
      {/* MOBILE MENU OVERLAY */}
      <div
        className={`lg:hidden fixed inset-0 z-[100] transition-all duration-500 ${isMenuOpen ? "visible opacity-100" : "invisible opacity-0"}`}
      >
        <div
        ...
        />
        <div
          className={`absolute left-0 top-0 h-full w-4/5 max-sm bg-white shadow-2xl transition-transform duration-500 ease-out transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
            ...
            <div className="mt-auto pt-10 border-t border-gray-100">
              {user ? (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-4 mb-4">
                    <img
                      src={user?.profileImage || avatarImg}
                      alt="avatar"
                      className="w-14 h-14 rounded-full border-2 border-orange-600 object-cover shadow-md"
                    />
                    <div>
                  ...
                  <Link
                    to="/profile"
                    onClick={() => setIsMenuOpen(false)}
                    className="w-full py-4 bg-gray-900 text-white rounded-xl font-black uppercase tracking-widest text-[10px] text-center"
                  >
                    {t('viewProfile')}
                  </Link>
                  <button
                    onClick={hanDelLogout}
                    className="w-full py-4 border border-red-100 text-red-600 rounded-xl font-black uppercase tracking-widest text-[10px]"
                  >
                    {t('signOut')}
                  </button>
                </div>
                ...
                <Link
                  to="/log-in"
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full py-5 bg-orange-600 text-white rounded-xl font-black uppercase tracking-widest text-center block shadow-lg shadow-orange-100"
                >
                  {t('signIn')}
                </Link>
              )}
              <button 
                onClick={() => { i18n.changeLanguage(i18n.language === 'en' ? 'bn' : 'en'); setIsMenuOpen(false); }}
                className="w-full mt-4 py-4 border border-gray-200 text-gray-900 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2"
              >
