    ...
    return (
      <div className="mb-24 last:mb-0" key={key}>
        <div className="flex items-center gap-4 mb-8 md:mb-10">
          <h3
            className={`text-xl md:text-3xl font-outfit font-black uppercase tracking-tight text-gray-900 px-4 md:px-5 py-2 ${themeColor === "orange" ? "bg-orange-50 border-orange-600" : "bg-amber-50 border-amber-500"} border-l-8 shadow-sm`}
          >
            {title}
          </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7">
            <Link
              ...
              className="group relative block bg-gray-900 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-700 overflow-hidden h-[350px] md:h-[450px] lg:h-full min-h-[300px] md:min-h-[400px]"
            >
              <img
              ...
              <div className="absolute bottom-0 left-0 p-6 md:p-12 w-full">
                <span
                  className={`${themeColor === "orange" ? "bg-orange-600" : "bg-amber-500"} text-[9px] md:text-[10px] font-outfit font-black uppercase tracking-[0.2em] px-3 md:px-4 py-1.5 rounded-full mb-4 md:mb-6 inline-block text-white shadow-lg`}
                >
                  {t("featuredStory")}
                </span>
            ...
            {secondaryBlogs.length > 0 ? (
              secondaryBlogs.map((blog) => (
                <Link
                  to={`/blogs/${blog._id}`}
                  key={blog._id}
                  className="group flex gap-5 bg-white p-4 rounded-2xl hover:bg-gray-50 transition-all duration-300 border border-gray-50 hover:border-gray-200 shadow-sm hover:shadow-md"
                >
                  ...
                  <div className="flex flex-col justify-center py-1">
                    <span
                      className={`text-[10px] font-outfit font-black uppercase tracking-[0.15em] ${themeColor === "orange" ? "text-orange-600" : "text-amber-500"} mb-2`}
                    >
                      {i18n.language === "bn"
                        ? blog.category_bn || blog.category || t("general")
                        : blog.category || t("general")}
                    </span>
                    <h4
                      className={`text-lg font-outfit font-bold text-gray-900 line-clamp-2 leading-tight group-hover:${themeColor === "orange" ? "text-orange-600" : "text-amber-500"} transition-colors duration-300`}
                    >
                      {i18n.language === "bn"
      ...
      {error && (
        <div className="bg-orange-50 text-orange-600 p-8 rounded-2xl border border-orange-100 text-center shadow-inner">
          ...
          <p className="text-sm opacity-80">
            {error?.data?.message || error?.error || t("errorFetchingBlogs")}
          </p>
        </div>
      )}
      {!isLoading && blogs.length === 0 && (
        <div className="bg-gray-50 text-gray-500 p-20 rounded-3xl border border-gray-200 text-center shadow-inner">
          <p className="text-2xl font-black uppercase tracking-tighter mb-4">
            {t("noStoriesFound")}
          </p>
